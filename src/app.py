...
from email.mime.text import MIMETeopenaixt
from fastai import *
...
def get_response():
    # System prompt to make it act like a dermatologist
    system_prompt = """You are an AI dermatology assistant. Your responses should:
    - Be professional and medically accurate
    - Focus on common skin conditions (acne, eczema, psoriasis, etc.)
    - Provide basic information about symptoms and treatments
    ...
    user_input = request.form.get("prompt_input")

    try:
        # OpenAI API call
        response = openai.chat.completions.create(
            model="gpt-3.5-turbo",  # or "gpt-4" for better medical accuracy
            messages=[
                {"role": "system", "content": system_prompt},
                {"role": "user", "content": user_input}
            ],
            temperature=0.3,  # Lower for more conservative medical responses
        ...
        return jsonify({"error": f"Sorry, I'm having trouble responding. Please try again later. Error: {str(e)}"}), 500
